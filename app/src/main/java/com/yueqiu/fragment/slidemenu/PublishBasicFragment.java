 ...
 */
public class PublishBasicFragment extends Fragment implements XListView.IXListViewListener{
    ...
    private void setEmptyViewText(){
        ...
        mEmptyView.setText(getString(R.string.your_published_info_is_empty,mTypeStr));
    }

    private void requestPublishedInfo(){
        mResultMap.put(DatabaseConstant.UserTable.USER_ID, YueQiuApp.sUserInfo.getUser_id());
        mResultMap.put(HttpConstants.Published.TYPE,mType + 1);
        mResultMap.put(HttpConstants.Published.START_NO,start_no);
        mResultMap.put(HttpConstants.Published.END_NO, end_no);
    ...
    private void updatePublishedInfoDB(){
        if(isExistPublished){
        ...
        public void handleMessage(Message msg) {
            switch(msg.what){
                case PublicConstant.USE_CACHE:
                    ...
                    mList.addAll(mPublishedInfo.mList);
                    mCachedIndex = mList.size();
                    break;
            ...
            mAdapter = new PublishedBasicAdapter(getActivity(),mList);
